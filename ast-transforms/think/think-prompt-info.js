...
export default function getPromptInfoFromThink(code, filePath, isJs = false) {
        ...
        ImportDeclaration(path) {
          for (let specifier of path.node.specifiers) {
            ...
            if (isPathFromReason(path.node.source.value)) {
              if (i.orginalname === 'reason') {
                specifier.imported.name = internalname + 'reason';
          
                path.scope.rename(internalname + 'reason', i.varname);
                thinkname = i.varname;
              }
            }
          }
        ...
        CallExpression(path) {
          if (t.isIdentifier(path.node.callee, { name: thinkname }) && (path.node.arguments.length === 1 || path.node.arguments.length === 2)) {
            ...
            if (path.node.typeParameters.params.length !== 1) {
              throw new ReasonError(`The \`think\` function can only receive one generic.`, 53)
            }
            ...
            if (param.type === 'TSTypeReference') {
              ...
              if (!codetypes.has(name)) {
                throw new ReasonError(`You are trying to use the interface/type \`${name}\` as the generic for the \`reason()\` call. But, sadly, RΞASON does not support importing types from other files yet. Please define the type in this file.\n\nThis will be coming in our next release.`, 554)
              }
